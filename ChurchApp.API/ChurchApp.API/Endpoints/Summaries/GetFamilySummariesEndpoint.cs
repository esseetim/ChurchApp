...
public sealed class GetFamilySummariesEndpoint(ChurchAppDbContext dbContext)
    : Endpoint<GetFamilySummariesRequest, SummariesResponse>
{
    public override void Configure()
    {
        Get("/api/summaries/family");
        AllowAnonymous();
    }

    public override async Task HandleAsync(GetFamilySummariesRequest req, CancellationToken ct)
    {
        var query = dbContext.Summaries
            .Where(x => x.Type == SummaryType.Family && x.FamilyId == req.FamilyId);
        ...
        if (req.PersistReport)
        {
            dbContext.Reports.Add(new Report
            {
                ...
                EndDate = req.EndDate,
                ParametersJson = JsonSerializer.Serialize(req, AppJsonSerializerContext.Default.GetFamilySummariesRequest),
